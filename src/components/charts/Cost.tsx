    ...
    services.forEach((service) => {
      const portion =
        (monthlyData[monthlyData.length - 1][
          service as keyof CostDataRecord
        ] as number) / monthlyData[monthlyData.length - 1].total;
...
export function CostAnalysis() {
  ...
  const currentMonthTotal =
    costData.monthlyData[costData.monthlyData.length - 1].total;
  const previousMonthTotal =
    costData.monthlyData[costData.monthlyData.length - 2].total;
  const percentChange =
  ...
  const getChartColors = () => {
    ...
    return {
      lambda:
        rootStyles.getPropertyValue("--color-chart-1").trim() || "#488fdf",
      apiGateway:
        rootStyles.getPropertyValue("--color-chart-2").trim() || "#10b981",
      s3: rootStyles.getPropertyValue("--color-chart-3").trim() || "#6366f1",
      dynamodb:
        rootStyles.getPropertyValue("--color-chart-4").trim() || "#f59e0b",
      cloudwatch:
        rootStyles.getPropertyValue("--color-chart-5").trim() || "#ef4444",
      sqs: rootStyles.getPropertyValue("--color-chart-1").trim() || "#488fdf",
      gridLines:
        rootStyles.getPropertyValue("--color-border").trim() ||
        "rgba(100, 100, 100, 0.2)",
      text:
        rootStyles.getPropertyValue("--color-muted-foreground").trim() ||
        "#64748b",
    };
  };
          ...
          <div className="space-y-2 text-sm">
            <div className="flex items-start space-x-2">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-xs">
                  Several Lambda functions have memory allocations much higher
                  than their actual usage. Consider reducing allocated memory to
                  save costs.
                </p>
              </div>
            </div>
            <div className="flex items-start space-x-2">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-xs">
                  Your consistent DynamoDB usage pattern suggests Reserved
                  Capacity would reduce costs by an estimated 27%.
                </p>
              </div>
            </div>
            <div className="flex items-start space-x-2">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-xs">
                  CloudWatch costs increased by 18% since last month. Review log
                  retention policies and debug logging levels.
                </p>
              </div>
            </div>
          </div>
