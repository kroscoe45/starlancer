...
const generateMockHealthData = () => {
  const services = [
    { 
      id: "lambda",
      name: "Lambda",
      health: Math.random() > 0.8 ? "degraded" : Math.random() > 0.9 ? "outage" : "operational",
      uptime: Math.random() > 0.8 ? 97 + Math.random() * 2 : 99 + Math.random() * 0.999,
      incidents: Math.floor(Math.random() * 3),
      latency: Math.random() > 0.7 ? 110 + Math.random() * 100 : 50 + Math.random() * 60,
      throughput: 1000 + Math.floor(Math.random() * 500),
      regions: [
        { name: "us-east-1", status: Math.random() > 0.9 ? "degraded" : "operational" },
        { name: "us-west-2", status: Math.random() > 0.95 ? "degraded" : "operational" },
        { name: "eu-west-1", status: "operational" },
        { name: "ap-southeast-1", status: Math.random() > 0.95 ? "outage" : "operational" }
      ]
    },
    { 
      id: "apigateway",
      ...
      uptime: Math.random() > 0.9 ? 98 + Math.random() * 1 : 99.5 + Math.random() * 0.499,
      incidents: Math.floor(Math.random() * 2),
      latency: Math.random() > 0.8 ? 150 + Math.random() * 150 : 70 + Math.random() * 80,
      throughput: 2000 + Math.floor(Math.random() * 1000),
      regions: [
        { name: "us-east-1", status: "operational" },
        { name: "us-west-2", status: Math.random() > 0.9 ? "degraded" : "operational" },
        { name: "eu-west-1", status: "operational" },
    ...
    { 
      ...
      uptime: Math.random() > 0.95 ? 99 + Math.random() * 0.5 : 99.8 + Math.random() * 0.199,
      incidents: Math.random() > 0.8 ? 1 : 0,
      latency: Math.random() > 0.9 ? 25 + Math.random() * 15 : 10 + Math.random() * 15,
      throughput: 5000 + Math.floor(Math.random() * 2000),
      regions: [
        { name: "us-east-1", status: "operational" },
        { name: "us-west-2", status: "operational" },
        { name: "eu-west-1", status: "operational" },
        { name: "ap-southeast-1", status: Math.random() > 0.9 ? "degraded" : "operational" }
      ]
    },
    { 
      id: "sqs",
    ...
    { 
      ...
      latency: Math.random() > 0.8 ? 60 + Math.random() * 60 : 30 + Math.random() * 30,
    ...
    { 
      ...
      name: "S3",
      health: Math.random() > 0.95 ? "degraded" : "operational",
      uptime: Math.random() > 0.95 ? 99.5 + Math.random() * 0.4 : 99.9 + Math.random() * 0.099,
      incidents: Math.random() > 0.9 ? 1 : 0,
      latency: Math.random() > 0.9 ? 80 + Math.random() * 70 : 40 + Math.random() * 40,
      throughput: 3000 + Math.floor(Math.random() * 1500),
      regions: [
        { name: "us-east-1", status: "operational" },
        { name: "us-west-2", status: "operational" },
        { name: "eu-west-1", status: Math.random() > 0.95 ? "degraded" : "operational" },
        { name: "ap-southeast-1", status: "operational" }
      ]
    }
  ];
...
export default function ServiceHealthSlide() {
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-border/80">
                <th className="text-left font-medium text-xs text-muted-foreground py-3 px-3">Service</th>
                <th className="text-left font-medium text-xs text-muted-foreground py-3 px-3">Status</th>
                <th className="text-left font-medium text-xs text-muted-foreground py-3 px-3 hidden md:table-cell">Uptime</th>
                <th className="text-left font-medium text-xs text-muted-foreground py-3 px-3 hidden lg:table-cell">Latency</th>
                <th className="text-left font-medium text-xs text-muted-foreground py-3 px-3 hidden lg:table-cell">Throughput</th>
              </tr>
            </thead>
            <tbody>
              {healthData.services.map((service) => (
                <tr key={service.id} className="border-b border-border/40 hover:bg-muted/30">
                  ...
                  <td className="py-3 px-3 hidden md:table-cell">
                    <div className="flex flex-col space-y-1">
                      ...
                      <Progress 
                        ...
                        className={
                          service.uptime >= 99.9 ? "bg-green-500/20" :
                          service.uptime >= 99 ? "bg-amber-500/20" : "bg-red-500/20"
                        }
                      />
                    </div>
                  </td>
                  <td className="py-3 px-3 hidden lg:table-cell">
                    <div className="flex items-center">
                      ...
                      <span className="text-xs">{service.latency.toFixed(0)} ms</span>
                      {service.latency > 100 && (
                  ...
                  <td className="py-3 px-3 hidden lg:table-cell">
                    <div className="flex items-center">
                      ...
                      <span className="text-xs">{service.throughput.toLocaleString()} req/s</span>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <CardContent className="p-4 pt-0">
            <div className="space-y-3 text-sm">
              {healthData.services
                ...
                .map(service => (
                  <div key={`incident-${service.id}`} className="flex items-start space-x-2">
                    ...
                    <div>
                      <p className="font-medium">{service.name} - {
                        service.health === "outage" ? "Service Disruption" : "Performance Degradation"
                      }</p>
                      <p className="text-muted-foreground text-xs">
                        {service.health === "outage" ? 
                          `Service is experiencing an outage in ${
                            service.regions.find(r => r.status === "outage")?.name || "some regions"
