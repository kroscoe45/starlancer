...
const generateMockData = (days = 7, type: string) => {
  ...
  const patterns: Record<
    string,
    { base: number; daytime: number; variance: number; trend: number }
  > = {
  ...
  for (let i = 0; i < days * 24; i++) {
    ...
    // Random variance
    const randomFactor =
      Math.random() * pattern.variance * 2 - pattern.variance;
    ...
    const value = Math.min(
      ...
      Math.max(
        0,
        (pattern.base + daytimeValue) * weekendFactor * trendFactor +
          randomFactor,
      ),
    );
...
export function ResourceUsageChart() {
  ...
  const getChartColors = (type: string) => {
    ...
    return {
      line:
        rootStyles.getPropertyValue(`--color-chart-${colorIndex}`).trim() ||
        "#488fdf",
      fill:
        rootStyles.getPropertyValue(`--color-chart-${colorIndex}`).trim() ||
        "#488fdf",
  ...
  const renderChart = (
    data: { timestamp: string; time: string; date: string; value: number }[],
    title: string,
    type: string,
  ) => {
